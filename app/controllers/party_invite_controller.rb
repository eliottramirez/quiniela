class PartyInviteController < ApplicationController
  def send_invite
    if params[:to_email].present?
      ...
      redirect_to party_path(Party.find(params[:party])), alert: "El campo \"Email\" no puede ir en blanco."
    end
  end
  ...
  def invite_params
    params.permit(:commit, :from_user, :party, :party_name, :sharing_code, :to_email)
  end
end
